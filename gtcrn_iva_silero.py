...
from silero import select_channel
...
class GTCRN_IVA(nn.Module):
    ...
    def forward(self, x):
        ...
        # WPE + IVA
        spec_drb = fd_wpe(spec_orig, rt60=0.3, shift=256, D=2, fs=self.fs, num_iter=1)
        spec_2ch = auxiva(spec_drb.transpose(1, 3), n_iter=10).transpose(1, 3) 

        # channel selection via Silero VAD (robust at both +/- SNR, unlike energy norm)
        chan0 = torch.istft(spec_2ch[:, 0], **stft_kwargs, length=n_samples)
        chan1 = torch.istft(spec_2ch[:, 1], **stft_kwargs, length=n_samples)
        pred = select_channel(chan0, chan1, fs=self.fs)
